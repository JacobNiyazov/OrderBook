...
bool Book::cancelOrder(std::shared_ptr<Order> order)
{
    ...
    if (order->getIsBuy())
    {
        ...
        if (lim->getSize() == 0)
        {
            buyTree = Limit::deleteLimit(buyTree, lim);
        }
    }
    else
    {
        ...
        if (lim->getSize() == 0)
        {
            sellTree = Limit::deleteLimit(sellTree, lim);
        }
...
bool Book::executeOrder(std::shared_ptr<Order> order)
{
    ...
    std::shared_ptr<Limit> parentLim = (order->getParentLimit()).lock();
    parentLim->setTotalVol(parentLim->getTotalVol() - (order->getShares() - shares));
